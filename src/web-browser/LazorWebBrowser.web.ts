...
export default {
    ...
    maybeCompleteAuthSession({ skipRedirectCheck }: { skipRedirectCheck?: boolean }): {
        ...
        if (!parent) {
            throw new Error(`The window cannot complete the redirect request because the invoking window doesn't have a reference to it's parent. This can happen if the parent window was reloaded.`);
        }
    ...
    async openAuthSessionAsync(
        ...
        if (popupWindow == null || popupWindow?.closed) {
            ...
            if (popupWindow) {
                ...
                throw new Error('Popup window was blocked by the browser or failed to open. This can happen in mobile browsers when the window.open() method was invoked too long after a user input was fired.');
            }
        }
...
async function generateStateAsync(): Promise<string> {
    if (!isSubtleCryptoAvailable()) {
        throw new Error(`The current environment doesn't support crypto. Ensure you are running from a secure origin (localhost/https).`);
    }
