...
const PAYER_PUBLICKEY = new anchor.web3.PublicKey('hij78MKbJSSs15qvkHWTDCtnmba2c1W4r1V22g5sD8w');
  ...
  const saveWallet = async (data: WalletInfo): Promise<WalletInfo> => {
    ...
    try {
      ...
      if (!smartWallet || !smartWalletAuthenticator) {
        ...
        logger.log('💸 Airdropping 0.1 SOL to new SmartWallet...');
        ...
        airdropTxn.recentBlockhash = (await lazorProgram.connection.getLatestBlockhash()).blockhash;
        airdropTxn.sign(payerKeypair);

        await lazorProgram.connection.sendRawTransaction(airdropTxn.serialize());
      }
